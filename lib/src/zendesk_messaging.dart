...
class ZendeskMessaging {
  ...
  /// Check if a Firebase push notification should be handled by Zendesk and display it.
  ///
  /// On iOS this always returns `false` — iOS uses APNs directly via the
  /// `UNUserNotificationCenterDelegate` in the native plugin.
  ///
  /// On Android, delegates to the native Zendesk SDK push notification handler.
  ///
  /// Prefer [shouldBeDisplayed] + [handleNotification] for new code.
  static Future<bool> checkAndDisplayFirebaseNotification(
    ...
    try {
      ...
      ZendeskMessagingConfig.logError(
        'checkAndDisplayFirebaseNotification failed',
        error: e,
        stackTrace: stackTrace,
      );
      return false;
  ...
  /// and you don't want double-notification UX).
  static Future<void> disablePushNotifications(
    bool pushNotificationsDisabled,
  ) async {
    try {
      await _channel.invokeMethod('disablePushNotifications', {
        'pushNotificationsDisabled': pushNotificationsDisabled,
      });
    } catch (e, stackTrace) {
      ZendeskMessagingConfig.logError(
        'disablePushNotifications failed',
        error: e,
        stackTrace: stackTrace,
      );
    }
  ...
  /// Enable or disable native Zendesk SDK logging.
  ///
  /// Useful to turn on in debug/dev builds for verbose output.
